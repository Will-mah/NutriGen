...
ingredient_pattern = re.compile(
    ...
    r'(?:\d+\s*\d*/\d+|\d*/\d+|\s*[\u00BC-\u00BE\u2150-\u215E])?'  # Matches whole plus fractional part of the quantity (including unicode fractions and spaces)
    r'[\s]*'  # Matches any spaces following the numbers
    r'(?:fl\.?\s?)?'  # Optionally matches 'fl' abbreviation for fluid measurements
    r'(?:g|oz|lbs?|kg|tbsp?|tsp|cups?|ml|l|pints?|quarts?|gallons?|litres?|liters?)'  # Matches the unit
    r'[\s\/]*'  # Matches spaces or slashes after the unit
    r'(?:\d*[\s]*'  # Matches an optional whole number for alternative measurements
    r'(?:\d+/\d+|\s*[\u00BC-\u00BE\u2150-\u215E])?'  # Matches fractional part of the alternative measurements (including unicode fractions)
    r'[\s]*'  # Matches any spaces following the alternative measurements
    r'(?:fl\.?\s?)?'  # Optionally matches 'fl' abbreviation for fluid measurements
    r'(?:g|oz|lbs?|kg|tbsp?|tsp|cups?|ml|l|pints?|quarts?|gallons?|litres?|liters?)?'  # Optionally matches the unit for alternative measurements
